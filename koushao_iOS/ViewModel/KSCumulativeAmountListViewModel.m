//
//  KSCumulativeAmountListViewModel.m
//  koushao_iOS
//
...
@interface KSCumulativeAmountListViewModel()

@property (nonatomic,strong) KSMoneyRecordListModel *model;

@end

@implementation KSCumulativeAmountListViewModel
...
- (RACSignal*)requestRemoteDataSignalWithId:(NSNumber *)mid refresh_type:(KSRequestRefreshType)refresh_type{
    RACSignal *fetchSignal =  [KSClientApi getMoneyRecordWithId:mid refresh_type:refresh_type record_type:KSMoneyRecordApiTypeObtain limit:@(self.perPage)];
...
- (NSArray*)dataSourceWithItems:(NSArray *)items {
    ...
    NSArray *viewModels = [items.rac_sequence map:^(KSMoneyRecordItemModel *item) {
        KSCumulativeAmountItemViewModel *viewModel = [[KSCumulativeAmountItemViewModel alloc] initWithServices:self.services params:@{@"item" : item}];
        return viewModel;
    }].array;
