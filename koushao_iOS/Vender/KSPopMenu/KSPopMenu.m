...
@interface KSPopMenu () <UITableViewDelegate, UITableViewDataSource>
...
- (UIImageView *)menuContainerView {
    if (!_menuContainerView) {
        ...
        _menuContainerView.frame = CGRectMake(CGRectGetWidth(self.bounds) - kCDMenuTableViewWidth - 6, 0, kCDMenuTableViewWidth, self.menus.count * (kCDMenuItemViewHeight + kCDSeparatorLineImageViewHeight) + kCDMenuTableViewSapcing);
...
- (instancetype)initWithObjects:(id)firstObj, ... NS_REQUIRES_NIL_TERMINATION {
    ...
    if (self) {
        ...
        KSPopMenuItem *eachItem;
        va_list argumentList;
        if (firstObj) {
            [menuItems addObject:firstObj];
            va_start(argumentList, firstObj);
            while((eachItem = va_arg(argumentList, KSPopMenuItem *))) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    KSPopMenuItemCell *cell = (KSPopMenuItemCell *)[tableView dequeueReusableCellWithIdentifier:cellIdentifer];
    if (!cell) {
        cell = [[KSPopMenuItemCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifer];
    }
