...
const static CGFloat kCustomIOSAlertViewDefaultButtonSpacerHeight = 0;
...
- (id)init
{
    ...
    if (self) {
        ...
        UITapGestureRecognizer *panRecognizer = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(handleTouch)];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if(i<self.buttonColors.count)
        {
            UIColor* buttonColor=[self.buttonColors objectAtIndex:i];
            if(buttonColor)
            {
