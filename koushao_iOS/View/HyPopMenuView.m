...
#define CancelStrImgaeName @"compose_add"
...
-(void)initUI
{
    ...
    CGFloat CANCELw = 40;
    ImageView *CancelButton = [[ImageView alloc] initWithFrame:CGRectMake(CGRectGetWidth(DownView.bounds)/2 - CANCELw/2, CGRectGetHeight(DownView.bounds)/2 - CANCELw/2+2, CANCELw, CANCELw)];
...
-(void)CirculatingItmes
{
    ...
    for (MenuLabel *Obj in _ItmesArr) {
        ...
        buttonX = (index==1?index+1:index % HowMucHline) * ButtonWidth;
        if(index==0)buttonX+=50;
        else if(index==1)buttonX-=50;
        ...
        CGRect toValue = CGRectMake(buttonX, CGRectGetMinY(downView.frame)-260, ButtonWidth, ButtonHigh);
        if (index == 0) {
...
-(void)DismissCompletionBlock:(void(^) (BOOL CompletionBlock)) completionBlock{
    ...
    for (MenuLabel *label in _ItmesArr) {
        ...
        CGRect toValue = CGRectMake(buttonX, CGRectGetMinY(downView.frame), ButtonWidth, ButtonHigh);
        CGRect fromValue = CGRectMake(buttonX, CGRectGetMinY(downView.frame)-260, ButtonWidth, ButtonHigh);
